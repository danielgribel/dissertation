...
%Important: The data point selected for relocation must belong to a cluster with more than one element, otherwise the empty cluster is populated but the cluster with one data point is left empty.
...
%To detect if a given solution $s$ has a clone in the population, we verify if for each solution $s'$ with ordered cardinalities $g'(i)$, such that $H(s')$ = $H(s)$ (having the same hash key of $s$), the following conditions must hold:
...
This characteristic of eliminating clones and poor solutions reveals two aspects of the \textit{Survivors selection} mechanism. The first one is that population diversity is maintained, as we favor the removal of clones first. The second is related to elitism, as the individuals with good fitness are preserved. %Other aspects related to diversity can be observed in mutation and diversification operators.
