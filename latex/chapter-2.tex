...
- for $m$ >= 2 and one dimensional data, MSSC can be solved in $O(n^3)$ time (Spath, 1980)

- for $m$ >= 2 and general dimension, MSSC is NP-hard (Aloise, Deshpande, Hansen and Popat, 2009) -- Euclidean space too

- If both $m$ and dimension $d$ are fixed, the problem can be solved in $O(n^{md+1})$ time (Inaba, Katoh and Imai, 1994), which may be very time-consuming.

- The hardness of MSSC is not directly measured by the number of entities, number of dimensions, number of clusters, etc. It also depends on the distribution of points (Hansen, Aloise, 2009).
